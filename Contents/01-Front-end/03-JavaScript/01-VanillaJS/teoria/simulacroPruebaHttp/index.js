...
function addedInfo(array) {
    ...
    document.querySelector(".owner img").src = array[0].owner.avatar_url
    document.querySelector(".login").innerHTML = array[0].owner.login
    document.querySelector(".bio").innerHTML = array[0].owner.bio
    document.querySelector(".repositores").innerHTML = `repositores`
    document.querySelector(".barra").innerHTML = `<div class="bar"></div>`
...
document.querySelector("#button-addon2").addEventListener("click", () => {
    ...
    let name = document.querySelector("body > div > div > div.input-group.mb-3 > input").value;
     ...
     httpGetAsync("https://cors-anywhere.herokuapp.com/https://api.github.com/users/" + name + "/repos", addedInfo);
});